...
import "@/styles/mob/pwd.register.scss";
import "@/styles/mob/trade.scss";
...
$(document).ready(function() {
  ...
  function setUserInfo(data) {
    ...
    if(data.status == 1 && data.data){
      ...
      // }
      // 冻结额
      $buyLock.attr('data-tormb', fixMulti(rmbPrice, $buyLock.text()));
      $sellLock.attr('data-tormb', fixMulti(rmbPrice, fixMulti(newestPrice, $sellLock.text())));
      // 余额
      $buyAvailUse.attr('data-tormb', fixMulti(rmbPrice, coinToOver) || phpLang.LOADING);
      $sellAvailUse.attr('data-tormb', fixMulti(rmbPrice, fixMulti(newestPrice, coinFromOver)) || phpLang.LOADING);
    ...
    var options = {
      ...
      success(res) {
        if (parseInt(res.status) === 1) {
          ...
          if (res.data.orders.ratio < 0) {
            $('.tab_datas .font_first').removeClass('font_red').addClass('font_green');
          }
          else if (res.data.orders.ratio > 0) {
            $('.tab_datas .font_first').removeClass('font_green').addClass('font_red');
          }
          else {
            $('.tab_datas .font_first').removeClass('font_red').removeClass('font_green').addClass('font_normal');
          }

          if (res.data.orders.price < 0) {
            $('.tab_datas .font_two').removeClass('font_red').addClass('font_green');
          }
          else if (res.data.orders.price > 0) {
            $('.tab_datas .font_two').removeClass('font_green').addClass('font_red');
          }
          else {
            $('.tab_datas .font_two').removeClass('font_red').removeClass('font_green').addClass('font_normal');
          }
          $('.tab_datas .font_first').text(res.data.orders.ratio+"%");
          if (!res.data.orders.price) {
            res.data.orders.price = 0;
          }
          $('.tab_datas .font_two').text(res.data.orders.price);
        }
        // 控制是否只更新一次
        if (!upDateOnce) {
          /**mob**/
  ...
  function setOrder(data) {
    ...
    if(data){
      ...
      newestPrice = data.price;   //全局對象最新成交價
      let max = data.max || 0;
      var min = data.min;
      var sum = data.sum;
      ...
      //最新成交价
      if(nonull(price)  && price !== ''){
        $newestPrice.html(price.int + "." + price.left + "<span class='low-vision'>" + price.dark + "</span>" + '<span>&nbsp;'+coinTo.toUpperCase() +'</span>');
        $newestPrice.attr('data-tormb', fixMulti(data.price, rmbPrice) || phpLang.LOADING);
        addHoverWin($('#newest-price'), 'tormb', 'bottom');     //最新成交价人民币悬浮框
      }
      //最高 价
      ...
      // }
      $lowestPrice.attr('data-tormb', fixMulti(max, rmbPrice));
      addHoverWin($lowestPrice, 'tormb', 'bottom');          //最低价人民币悬浮框
      //成交量
      if(nonull(sum)  && sum !== ''){
      ...
      //成交额
      if(nonull(volumn)){
        $dayVolumn.attr('data-tormb', fixMulti(rmbPrice, volumn)).html(volumn + '<span>&nbsp;'+coinTo.toUpperCase() +'</span>');
      }
      // 渲染每條成交數據
      if (nonull(orders)){
  ...
  */
  ...
  function setTrust(data) {
    if (data) {
      ...
      if (is(trustSellArr, "Array") && trustSellArr.length > 0) {
        // 判断数据是否超过限制条数
        if (trustSellArr.length > 5) {
          trustSellArr = trustSellArr.slice(-5);
        }
        ...
        trustSellArr.forEach(function(element, i) {//循環賣出委托數組
          //传来的数组长度与表格可容纳的列的差
          // var diff = (trustSellArr.length - sellItem) > 0 ? (trustSellArr.length - sellItem) : 0;
          //数组前diff个不显示(卖出委托根据累积量从大到小排序，返回的数据过多情况下，要保证最小的能显示)
          // if(i < diff) return;
          // const amount = nonull(element.n) ? beautyNum(element.n) : beautyNum(0, 9);
          const amount = element.n ? fromatNum(element.n, 9) : {int: '0', left: '', dark: "00000000"};
          // const price = nonull(element.p) ? beautyNum(element.p, 9) : beautyNum(0, 9);
  ...
  function setMyTrust(data) {
    ...
    if(data && data.length > 0) {
      ...
      function newTr(element) {
        return `<tr data-orderId="${element.id}" data-tormb="${fixMulti(rmbPrice, element.price) || phpLang.LOADING}">
        <td>${element.created.slice(-8)}</td>
      ...
      if (trusTrsLen > 0) {
        ...
        myTrustArr.forEach((element, i) => {
          ...
          if (i < trusTrsLen) {
            ...
            // 委託時間
            tds[0].innerHTML = element.created.slice(-8);
  ...
  function trClick(e) {
    ...
    if($thisTbody.attr('id') === 'sellout-list') {
      //填充价格并赋予人民币属性
      // $inputBuy.val(price).attr('data-tormb', $this.attr('data-tormb'));
      // $inputBuy.val($priceTd[0].firstChild.nodeValue).attr('data-tormb', $this.attr('data-tormb'));

      computeMaxBuyNum('#form-buy', $inputBuy.val());
    ...
    //市場委托买入
      ...
      computeMaxBuyNum('#form-sell', $inputSell.val());
  ...
  // 點擊數據填充到下單表格中
  // $('.trade-table').on('mousedown', 'tr', trClick);
  // 移动端
  $('.trade-table').on('touchstart', 'tr', trClick);
  /**獲取表單的值
  ...
  $('input[data-input="trade"]').on('keyup', function(){
    ...
    $this.attr('data-tormb', fixMulti(rmbPrice, $this.val()));   //添加兑换人民币属性
    ...
    timer_form = setTimeout(function() {
      ...
      if (data) {
        ...
        $.ajax({
          ...
          error: function(err){
            toast(phpLang['NET_ERROR'], $this, 'toast-ban'); //網路故障
            $submitBtn.removeAttr('disabled').parent().removeClass('is-submitting');
          }
        })
      }
    ...
    if(!Number(availUse)){
      ...
      if($thisForm.attr('id') === 'form-buy'){
        ...
        if(isNaN(fixMulti((ratioVal/100), availBuy))){
          ...
          var lastInputVal = eToNum(fixMulti((ratioVal/100), availBuy));
      ...
      //卖出
        ...
        if (isNaN(fixMulti((ratioVal/100), availUse))) {
          ...
          var lastInputVal = eToNum(fixMulti((ratioVal/100), availUse));
          $lastInput.val(cutFixedNum(lastInputVal));
        }
      }
      //計算交易額
  ...
  // 切换选项
  $("[data-where]").click (function() {
    let $this = $(this);
    $this.siblings('span').removeClass('act-t');
    $this.addClass("act-t");
    let tog = $this.attr('data-where');
    // 获取表格
    let $top = $this.parents("[data-top]");
    $top.find("[data-tog]").hide();
    $top.find('table.trade-table tbody').hide();
    $top.find('table.trade-table thead').hide();

    $("#"+tog).show();
    $top.find(`[data-tog="${tog}"]`).show();
    // console.log($(this).attr('data-where'));
  });

  // 点击币切换买卖form
  $("#buySellTable tbody tr").on("touchstart", function() {
    let tBodyId = $(this).parent("tbody").attr("id");
    $("#buySellTable tbody .hove-tr").removeClass("hove-tr");
    $(this).addClass('hove-tr');
    // 买入 （对应表单 需要反过来）
    if (tBodyId.indexOf('buy') > -1) {
      $("[data-where='formSell']").click();
    }
    // 卖出（对应表单 需要反过来）
    else {
      $("[data-where='formBuy']").click();
    }
  });
  ...
  // $("#coinOptions").on('touchmove',function() {
  //   isMoveTouch = true;
  // });
  // $("#coinOptions p").on("touchend", function() {
  ...
  //   //
  //   isMoveTouch = false;
  //   window.location.href = `/trade/${coinName.toLowerCase().replace('/', '_')}`;
  // });
  //
  // // 选择 币 切换币来源
  ...
  //     this.init(options);
  //     if (options.message) {
  //       this.message = clone(options.message)
  //     }
  //   }
  //   init (options) {
  ...
  //   }
  //   open(sendData, opFn) {
  //     let sdData = this.data || sendData;
  //     //
  //     this.socket.addEventListener("open", (evt) => {
  //       // this.send()
  //       this.socket.send(sdData);
  //     });
  //   }
  //   send(data) {
  //     this.sendData = data;
  ...
  //       if (callback && Object.prototype.toString.call(callback).indexOf("Function") > -1) {
  //         callback(event.data);
  //       }
  //       if (this.message) {
  ...
  // }
  // //
  // const sockfn = new socketApi({
  //   url: "ws://192.168.20.136:8001",
  //   data: JSON.stringify({
  //     token: cookie.getItem("USID"),
  //     channel: `${coinFrom}_${coinTo}`
  //   }),
  //   message(event) {
  //     console.log(event);
  //   }
  ...
  //点击选择部分 下拉框出现
  $(".tab_chosen").click(function() {
    if ($('.tab_select_chosen').css('display') === 'none') {
      $('.tab_select_chosen').show();
    }
    else {
      getTradeDatas();
      $('.tab_select_chosen').hide();
    }
  })
  $('body').click(function(e) {
    const tar = e.target;
    if ($(tar).parents('.tab_select_chosen').length == 0 && $(tar).parents('.tab_chosen').length == 0 && !$(tar).hasClass('tab_select_chosen') && !$(tar).hasClass('tab_chosen')) {
      $('.tab_select_chosen').hide();
    }
  })
  //点击交易区切换样式
  $('body').on('click', 'li.tab_area_key', function() {
      const coinName = $(this).attr("data-coinname");
      getTradeDatas(coinName.toLowerCase());
  })
  //点击数据列表跳转页面
  $('body').on('click', '.tab_select_right table tbody tr', function() {
    const coinFrom = $(this).find('td').eq(0).text().split('/')[0];
    const coinTo = $(this).find('td').eq(0).text().split('/')[1];
    window.location.href = "/trade/"+ coinFrom.toLowerCase() + '_' + coinTo.toLowerCase() ;
  })

 getTradeDatas();

  //交易区下拉框
  function getTradeDatas (coinCode) {
    if (coinCode) {
      coinCode = coinCode;
    }
    else {

      coinCode = $('.tab_chosen').text().split('/')[1].toLowerCase();
    }
    $.ajax({
      url: "/ajax_market/getAllQuote",
      method: "POST",
      ...
      success: function(result) {
        if (result && parseInt(result.status) === 1) {
          const { data } = result;
          getRightDatas(result, coinCode);
          $('.tab_select_left ul li.tab_area_key').remove();
          Object.keys(data).forEach((coinName) => {
            const leftList = tradeArea(coinName.toUpperCase(), coinCode.toUpperCase());
            $('.tab_select_left ul').append(leftList);
          });
        }
      }
    })
  ...
  //储存右边数据
  function getRightDatas(result, coinCode) {
    const { data } = result;
    $('.tab_select_right table tbody>tr').remove();
    Object.keys(data[coinCode]).forEach((coinName) => {
      const ListHtml = tradeDataList(coinName, data[coinCode][coinName]);
      $(".tab_select_right table tbody").append(ListHtml);
    });
  }
  //交易区选择部分
  function tradeArea(coinName, coinCode) {
    return `
    <li class="tab_area_key ${coinCode == coinName ? 'tab_area_active': ''}" data-coinname="${coinName}">${coinName}${phpLang["TRADE_AREA_TITLE"]}</li>
    `;
  }
  //交易區数据列表
  function tradeDataList(coinName, datas) {
    return `
    <tr>
      <td class="th_left">${coinName.replace('_', '/').toUpperCase()}</td>
      <td class="${datas['ratio'] < 0 ? 'font_green' : datas['ratio'] ? 'font_red' : ''}">${datas['ratio'] < 0 ? datas['ratio'] : "+"+datas['ratio']}%</td>
      <td class="th_right">${datas['amount']}</td>
    </tr>
    `;
  }
