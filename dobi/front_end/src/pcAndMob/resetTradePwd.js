  ...
  methods: {
    ...
    isTheSameAsLoginPwd() {
      http({
        method: 'POST',
        data: {
          pwd1: this.formDate.pwd1
        },
        success: ({status, data, msg}) => {
          let intStatus = parseInt(status);
          if (intStatus != 1) {
            this.dataTips.pwd1 = 3;
            this.errMsg.pwd1 = msg;
          }
        }
      })
    },
    ...
    setTradePwdFn() {
      if (this.isSubmit) return;
      this.isSubmit = true;
      if (!this.inputBlur('pwd1') || !this.inputBlur('pwd2')) {
        return this.isSubmit = false;
      }
