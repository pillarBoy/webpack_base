...
import "@/styles/mob/user.coinout.scss";
...
$(document).ready(function() {
  ...
  myBox.tryCallback = function() {
    // $("body").removeClass("body-over-hide");
    // $(".bg-content").removeClass("coin-out-body");
    $("#sectionBody").show();
  }

  // myBox.css({ height: '714px' });
  // 切換語言
  ...
  function coinOutBox(coin) {
    // $("body").addClass("body-over-hide");
    // $(".bg-content").addClass("coin-out-body");
    $("#sectionBody").hide();


    const coinOutBoxTpl = `<div class="coin-out-form">
      ...
      <div class="mut-tab last-tab"><button type="button" class="coin-out-submit" id="formCoinOutBtn">${phpLang.coinOutTil}<u></u></button></div>
      <p class="dot-line"></p>
      ...
      function checkInput($input, name, val, isSubmit) {
        ...
        if (!val) {
          ...
          if (name === 'number') {
            ...
            // 判断是否在规则范围内
            if (math.compare(parsNum, minout) < 0 || math.compare(maxout, parsNum) < 0) {
              tipWinCtrl($input, false, 'over');
              result = false;
            } else
            // 余額不足
            if (math.compare(balance, parsNum) < 0) {
      ...
      $("#showRealRMB").on("input", function () {
        ...
        if (val){
          ...
          if (!realMoneyRule.test(val)) {
            // 提取数字部分
            val.replace(/[0-9]+(?:\.([0-9]*)?)/g, function(key) {
              // 整数部分 （没有小数点） 是否超出 10 位
              if (key.indexOf('.') === -1 && key.split('.')[0].length > 10) {
                $input.val(key.slice(0,10));
              } else {
                $input.val(key);
              }
            });
  ...
  // 提币
  $("#coinOutBtn").click(coinOutFun);
  // $("#coinOutBtn").on("touchend", coinOutFun);
  //   function(e) {
  //
  // });
  // 切换
  ...
  /* eslint-disable */
  // 幣篩選條件
  // $('#coinList span[data-coincode]').click(function(e) {
  //   e.stopPropagation && e.stopPropagation();
  //   console.log(12312);
  //   // console.log(1414);
