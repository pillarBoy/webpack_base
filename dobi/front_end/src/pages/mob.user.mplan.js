...
// mob
import "@/styles/mob/user.mplan.scss";
...
$(document).ready(function() {
  ...
  $('[data-copy="ele"]').click(function(e) {
    ...
    try {
      ...
      if (succ) {
        myAlert.show(language['MOB_COPY_SUCCESS']); //復製成功
      } else {
        myAlert.show(language['MOB_COPY_NOT_SUPPORT']); //複製不成功，不支持
      }
    } catch (e) {
      if (e) {
        myAlert.show(language['MOB_COPY_NOT_SUPPORT_UPDATE']); //複製不成功，升級瀏覽器
      }
    }
    transfer.blur();
  });
  ...
  function coinOutBox(coin) {
    ...
    myBox.show(coinOutBoxTpl, function() {
      ...
      //   myBox.content.find(`[data-ccoin="${coinName}"]`).fadeIn('fast');
      //   // 清空错误提示
      ...
      function selCoin() {
        ...
        areaType = $(this).attr('data-ctype');
        console.log(coinTypes);
        let coinName = coinTypes;
        coinOutType = coinTypes;
        $('.mob-rf-content').parent('.top-tab').removeClass('warn-outline');
        $('.mob-rf-content').siblings('.warn-tips-lf').removeClass('err-tips').hide();
        // 切换提示内容
        myBox.content.find('[data-ccoin]').hide();
      ...
      $("#formCoinOutBtn").click(function() {
        ...
        const postData = {};
        // 獲取提幣類型
        let sel = myBox.content.find('.sel-coin');
        if (!sel.attr('data-coincode')) {
          $('.mob-rf-content').parent('.top-tab').addClass('warn-outline');
          $('.mob-rf-content').siblings('.warn-tips-lf').addClass('err-tips').show();
        }
        postData.coin = sel.attr('data-coincode');
        // 獲取所有輸入框
