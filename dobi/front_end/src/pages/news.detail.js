        ...
        if (is(data, 'Array') && data.length > 0) {
          ...
          function monents(time) {
            ...
            return {
              ...
              hh: timeObj.getHours() + ":" + timeObj.getMinutes()
            }
          }
          // 评论显示时间
          ...
          data.reverse().forEach((iss) => {
            issListStr += `<li>
              <div class="iss-li-top">
                <span>${iss.mo.slice(0,3)}****${iss.mo.slice(-4)}</span><span>${timeFilter(iss.created)}</span>
              </div>
